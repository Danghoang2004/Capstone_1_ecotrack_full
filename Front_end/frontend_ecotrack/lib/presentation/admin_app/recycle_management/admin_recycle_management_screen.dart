...
class AdminRecycleManagementScreen extends StatefulWidget {
  ...
  @override
  State<AdminRecycleManagementScreen> createState() => _AdminRecycleManagementScreenState();
}

class _AdminRecycleManagementScreenState extends State<AdminRecycleManagementScreen> {
  ...
  List<RecycleGuideDto> _filtered() {
    ...
    return _guides.where((g) => g.name.toLowerCase().contains(q) || g.description.toLowerCase().contains(q)).toList();
  }
  ...
  List<RecycleGuideDto> _getPageItems(List<RecycleGuideDto> source, int safePage) {
  ...
  Map<String, dynamic> _getCategoryInfo(String wasteTypeKey, String wasteTypeLabel) {
    switch (wasteTypeKey.toLowerCase()) {
      case 'nhua':
        return {'name': wasteTypeLabel, 'color': const Color(0xFF4CAF50), 'icon': Icons.shopping_bag_outlined};
      case 'giay':
        return {'name': wasteTypeLabel, 'color': const Color(0xFF66BB6A), 'icon': Icons.description_outlined};
      case 'kim loai':
        return {'name': wasteTypeLabel, 'color': const Color(0xFF42A5F5), 'icon': Icons.category_outlined};
      case 'pin':
        return {'name': wasteTypeLabel, 'color': const Color(0xFFFFA726), 'icon': Icons.battery_charging_full_outlined};
      case 'thuy tinh':
        return {'name': wasteTypeLabel, 'color': const Color(0xFF66BB6A), 'icon': Icons.water_drop_outlined};
      case 'vai':
        return {'name': wasteTypeLabel, 'color': const Color(0xFFAB47BC), 'icon': Icons.checkroom_outlined};
      case 'dien tu':
        return {'name': wasteTypeLabel, 'color': const Color(0xFF29B6F6), 'icon': Icons.devices_outlined};
      case 'huu co':
        return {'name': wasteTypeLabel, 'color': const Color(0xFFFFCA28), 'icon': Icons.eco_outlined};
      case 'dau an':
        return {'name': wasteTypeLabel, 'color': const Color(0xFFFFB300), 'icon': Icons.local_drink_outlined};
      default:
        return {'name': wasteTypeLabel, 'color': const Color(0xFF90A4AE), 'icon': Icons.recycling_outlined};
    }
  }
  ...
  Future<void> _showGuideDialog({RecycleGuideDto? guide}) async {
    ...
    final difficultyCtrl = TextEditingController(text: guide?.difficultyLevel ?? '');
    final timeCtrl = TextEditingController(text: guide?.estimatedTimeMinutes?.toString() ?? '');
    final materialsCtrl = TextEditingController(text: guide?.materialsNeeded ?? '');
    final List<_StepDraft> stepDrafts =
        guide != null && guide.steps.isNotEmpty
            ? guide.steps
                .map(
                  (step) => _StepDraft(
    ...
    final List<String?> stepTitleErrors = List<String?>.filled(stepDrafts.length, null, growable: true);
    final List<String?> stepDescriptionErrors = List<String?>.filled(stepDrafts.length, null, growable: true);
    ...
    Future<void> pickAndUploadToController(
      ...
      if (bytes == null) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Không đọc được file đã chọn'), backgroundColor: Colors.red),
        );
        return;
      }
      ...
      try {
        ...
        final url = await _service.uploadMedia(bytes: bytes, fileName: fileName, folder: folder);
        dialogSetState(() {
          controller.text = url;
          imageError = null;
        });
          ...
          return AlertDialog(
            ...
            title: Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.recycling_outlined, color: AppColors.adminAccent),
                ),
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('Loại rác', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                    ...
                    const Text('Tên', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                    const SizedBox(height: 8),
                    TextField(
                      controller: nameCtrl,
                      onChanged: (_) {
                    ...
                    const SizedBox(height: 14),
                    const Text('Ảnh đại diện hướng dẫn', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                    const SizedBox(height: 8),
                    TextField(
                    ...
                    const SizedBox(height: 14),
                    const Text('Độ khó', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                    const SizedBox(height: 8),
                    TextField(
                    ...
                    const SizedBox(height: 14),
                    const Text('Thời gian ước tính (phút)', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                    const SizedBox(height: 8),
                    TextField(
                      controller: timeCtrl,
                      decoration: InputDecoration(
                    ...
                    const SizedBox(height: 14),
                    const Text('Nguyên liệu cần thiết', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                    const SizedBox(height: 8),
                    TextField(
                    ...
                    ...List.generate(stepDrafts.length, (index) {
                      ...
                      return Container(
                        key: ValueKey('step_${index}_${step.titleCtrl.hashCode}'),
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              children: [
                                Text('Bước ${index + 1}', style: const TextStyle(fontWeight: FontWeight.w700)),
                                const Spacer(),
                            ...
                            TextField(
                              ...
                              onChanged: (_) {
                                if (stepTitleErrors[index] != null) {
                                  setStateDlg(() => stepTitleErrors[index] = null);
                                }
                              },
                              decoration: InputDecoration(
                            ...
                            TextField(
                              ...
                              onChanged: (_) {
                                if (stepDescriptionErrors[index] != null) {
                                  setStateDlg(() => stepDescriptionErrors[index] = null);
                                }
                              },
                              decoration: InputDecoration(
                              ...
                              child: OutlinedButton.icon(
                                onPressed: (saving || uploadingMedia)
                                    ...
                                    : () => pickAndUploadToController(
                                          ...
                                          fileType: FileType.image,
                                      dialogSetState: setStateDlg,
                                          folder: 'ecotrack/recycle/step-images',
                                        ),
                              ...
                              child: OutlinedButton.icon(
                                onPressed: (saving || uploadingMedia)
                                    ...
                                    : () => pickAndUploadToController(
                                      ...
                                      dialogSetState: setStateDlg,
                                          folder: 'ecotrack/recycle/step-videos',
                                        ),
            ...
            actions: [
              TextButton(
                ...
                child: const Text('Hủy', style: TextStyle(color: AppColors.adminTextSecondary, fontWeight: FontWeight.w600)),
              ),
              ElevatedButton(
                onPressed: saving
                    ? null
                    : () async {
                        ...
                        if (selectedWasteKey == null || selectedWasteKey!.isEmpty) {
                        ...
                        if (materialsCtrl.text.trim().isEmpty) {
                          materialsError = 'Vui lòng nhập nguyên liệu cần thiết';
                          hasValidationError = true;
                        }
                        ...
                        for (int i = 0; i < stepDrafts.length; i++) {
                          ...
                          if (step.descriptionCtrl.text.trim().isEmpty) {
                            stepDescriptionErrors[i] = 'Vui lòng nhập mô tả bước';
                            hasValidationError = true;
                          }
                        }
                        ...
                        try {
                          ...
                          for (int i = 0; i < stepDrafts.length; i++) {
                            ...
                            stepsPayload.add({
                              ...
                              'stepDescription': step.descriptionCtrl.text.trim(),
                              'instructionImageUrl': step.imageUrlCtrl.text.trim().isEmpty ? null : step.imageUrlCtrl.text.trim(),
                              'instructionVideoUrl': step.videoUrlCtrl.text.trim().isEmpty ? null : step.videoUrlCtrl.text.trim(),
                            });
                          }

                          final wasteLabel = wasteTypes.firstWhere((t) => t['key'] == selectedWasteKey)['label'] ?? selectedWasteKey;
                          final payload = {
                            ...
                            'difficultyLevel': difficultyCtrl.text.trim().isEmpty ? null : difficultyCtrl.text.trim(),
                            'estimatedTimeMinutes': int.tryParse(timeCtrl.text.trim()) ?? 0,
                            'materialsNeeded': materialsCtrl.text.trim(),
                            'steps': stepsPayload,
                          };
                          if (isEdit) {
                            await _service.updateGuide(guide!.guideId, payload);
                          } else {
                ...
                child: saving
                    ...
                    : Text(isEdit ? 'Lưu' : 'Thêm', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
              ),
            ],
          );
        });
        ...
        title: Row(
          children: [
            Container(
              ...
              child: Icon(catInfo['icon'] as IconData, color: catInfo['color'] as Color, size: 18),
            ),
          ...
          child: ConstrainedBox(
            ...
            child: SingleChildScrollView(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(guide.name, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700)),
                  ...
                  Container(
                    ...
                    child: Text(
                      ...
                      style: TextStyle(fontWeight: FontWeight.w600, color: catInfo['color'] as Color),
                    ),
                  ),
                  const SizedBox(height: 10),
                  if (guide.imageUrl != null && guide.imageUrl!.trim().isNotEmpty)
                    ClipRRect(
                      borderRadius: BorderRadius.circular(12),
                      child: Image.network(
                        guide.imageUrl!.trim(),
                  ...
                  const SizedBox(height: 6),
                  Text((guide.materialsNeeded ?? '').trim().isEmpty ? '(Chưa có)' : guide.materialsNeeded!),
                  const SizedBox(height: 10),
                  const Text('Các bước', style: TextStyle(fontWeight: FontWeight.w700)),
                  const SizedBox(height: 8),
                    ...
                    ...guide.steps.map(
                      (step) => Container(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Bước ${step.stepOrder}: ${step.stepTitle}', style: const TextStyle(fontWeight: FontWeight.w700)),
                            const SizedBox(height: 4),
                            Text(step.stepDescription),
                            if ((step.instructionImageUrl ?? '').trim().isNotEmpty) ...[
                              const SizedBox(height: 8),
                              ClipRRect(
                                borderRadius: BorderRadius.circular(8),
                                child: Image.network(
                                  ...
                                  errorBuilder: (_, __, ___) => Container(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Icon(Icons.image_not_supported_rounded, color: AppColors.adminTextSecondary, size: 32),
                                        const SizedBox(height: 6),
                                        const Text(
                                          'Không tải được ảnh',
                                          style: TextStyle(fontSize: 12, color: AppColors.adminTextSecondary),
                                        ),
                                      ],
                                    ),
                                  ),
                            ...
                            if ((step.instructionVideoUrl ?? '').trim().isNotEmpty) ...[
                              const SizedBox(height: 8),
                              Row(
                                children: [
                                  const Icon(Icons.video_library_outlined, size: 16, color: AppColors.adminTextSecondary),
                                  ...
                                  Expanded(
                                    child: Tooltip(
                                      ...
                                      child: GestureDetector(
                                        onTap: () {
                                          final url = step.instructionVideoUrl!.trim();
  ...
  Future<void> _deleteGuide(int id) async {
    final confirm = await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: const Text('Bạn có chắc chắn muốn xóa hướng dẫn này không?'),
            actions: [
              TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Hủy')),
              ElevatedButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Xóa'), style: ElevatedButton.styleFrom(backgroundColor: Colors.red)),
            ],
          ),
        ) ??
        false;
  ...
  Widget build(BuildContext context) {
    ...
    final allSelected = pageItems.isNotEmpty && pageItems.every((g) => _selectedGuideIds.contains(g.guideId));
    final someSelected = pageItems.any((g) => _selectedGuideIds.contains(g.guideId));
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            ...
            style: TextStyle(fontSize: 13, color: AppColors.adminTextSecondary, fontWeight: FontWeight.w500),
          ),
          const SizedBox(height: 18),
          Row(
            children: [
              Expanded(
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon: const Icon(Icons.search_rounded, color: AppColors.adminTextSecondary),
                    hintStyle: const TextStyle(color: AppColors.adminTextSecondary, fontWeight: FontWeight.w500),
                    filled: true,
                    fillColor: AppColors.adminSurface,
              ...
              ElevatedButton.icon(
                ...
                label: const Text('Thêm mới', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
                style: ElevatedButton.styleFrom(
                  backgroundColor: AppColors.adminAccent,
              ...
              child: Row(
                children: [
                  ...
                  Text(
                    ...
                    style: const TextStyle(color: AppColors.adminTextSecondary, fontWeight: FontWeight.w600),
                  ),
                  const SizedBox(width: 12),
                  ElevatedButton.icon(
                ...
                : filtered.isEmpty
                    ? Center(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(Icons.recycling_outlined, size: 64, color: AppColors.adminBorder),
                            const SizedBox(height: 16),
                            const Text(
                              'Chưa có hướng dẫn nào',
                              style: TextStyle(fontSize: 16, color: AppColors.adminTextSecondary, fontWeight: FontWeight.w600),
                            ),
                            const SizedBox(height: 8),
                            const Text(
                              ...
                              Container(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    const Expanded(
                                      flex: 2,
                                      child: Text('Hướng dẫn', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
                                    ),
                                    const Expanded(
                                      flex: 2,
                                      child: Text('Mô tả', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
                                    ),
                                    const Expanded(
                                      flex: 1,
                                      child: Center(
                                        child: Text('Số bước', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
                                      ),
                                    ),
                                    const SizedBox(
                                      width: 80,
                                      child: Center(
                                        child: Text('Mã', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
                                      ),
                                    ),
                                    const SizedBox(
                                      width: 170,
                                      child: Center(
                                        child: Text('Hành động', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
                                      ),
                                    ),
                                  ],
                                ),
                                  ...
                                  child: Column(
                                    children: pageItems
                                        .map((guide) {
                                          ...
                                          return Container(
                                            decoration: BoxDecoration(
                                              border: Border(bottom: BorderSide(color: AppColors.adminBorder.withOpacity(0.3))),
                                            ),
                                            child: Padding(
                                              ...
                                              child: Row(
                                                children: [
                                                  SizedBox(
                                                    width: 48,
                                                    child: Checkbox(
                                                      value: _isGuideSelected(guide.guideId),
                                                      onChanged: (v) => _toggleGuideSelection(guide.guideId, v ?? false),
                                                    ),
                                                  ),
                                                  const SizedBox(width: 12),
                                                  Expanded(
                                                    ...
                                                    child: Row(
                                                      children: [
                                                        Container(
                                                          ...
                                                          child: (guide.imageUrl != null && guide.imageUrl!.trim().isNotEmpty)
                                                              ? Image.network(
                                                                  guide.imageUrl!.trim(),
                                                                  fit: BoxFit.cover,
                                                  ...
                                                  SizedBox(
                                                    ...
                                                    child: Center(
                                                      child: Text(
                                                        '#${guide.guideId}',
                                                        style: const TextStyle(fontSize: 12, color: AppColors.adminTextSecondary, fontWeight: FontWeight.w600),
                                                      ),
                                                    ),
                                                  ),
                                                  SizedBox(
                                                    ...
                                                    child: Row(
                                                      ...
                                                      children: [
                                                        InkWell(
                                                          borderRadius: BorderRadius.circular(8),
                                                          onTap: () => _showGuideDetailsDialog(guide),
                                                          child: Container(
                                                            ...
                                                            child: const Icon(Icons.visibility_rounded, size: 18, color: Color(0xFF2196F3)),
                                                          ),
                                                        ),
                                                        const SizedBox(width: 8),
                                                        InkWell(
                                                          borderRadius: BorderRadius.circular(8),
                                                          onTap: () => _showGuideDialog(guide: guide),
                                                          child: Container(
                                                            padding: const EdgeInsets.all(8),
                                                            decoration: BoxDecoration(
                                                            ...
                                                            child: const Icon(Icons.edit_note_rounded, size: 18, color: Color(0xFF5F6B6B)),
                                                          ),
                                                        ),
                                                        const SizedBox(width: 8),
                                                        InkWell(
                                                          borderRadius: BorderRadius.circular(8),
                                                          onTap: () => _deleteGuide(guide.guideId),
                                                          child: Container(
                                                            padding: const EdgeInsets.all(8),
                                                            decoration: BoxDecoration(
      ...
      child: Row(
        ...
        children: [
          Text(
            ...
            style: const TextStyle(fontWeight: FontWeight.w600, color: AppColors.adminTextPrimary),
          ),
          Row(
            children: [
              IconButton(
                icon: const Icon(Icons.chevron_left),
                onPressed: safePage > 1 ? () => _setPage(safePage - 1, totalPages) : null,
                iconSize: 20,
              ),
              IconButton(
                icon: const Icon(Icons.chevron_right),
                onPressed: safePage < totalPages ? () => _setPage(safePage + 1, totalPages) : null,
                iconSize: 20,
              ),
            ],
          ),
